...
// Inject the initialization call
const initCall = `
        ...
        <div class="bg-white rounded-2xl shadow-sm hover:shadow-xl border border-gray-100 transition-all duration-300 overflow-hidden flex flex-col group">
          ...
          <div class="p-4 flex flex-col gap-2 flex-1">
            ...
            <div class="mt-auto pt-2 flex items-center justify-between">
              ...
              <button onclick="\${outOfStock ? '' : (p.has_variants ? \`location.href='\${prodUrl}'\` : \`Cart.add({id:\${p.id}, name:'\${escapeHtml(p.name).replace(/'/g,"\\\\'")}', price:\${p.price}, image:'\${mainImg}'}, 1); showToast('تمت إضافته إلى السلة بنجاح');\`)}" class="\${outOfStock ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : 'bg-blue-600 text-white hover:bg-blue-700 shadow-md hover:shadow-lg hover:-translate-y-0.5'} add-to-cart w-9 h-9 rounded-full flex items-center justify-center transition-all duration-300">
...
console.log('Successfully injected Related Products logic');
