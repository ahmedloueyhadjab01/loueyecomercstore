...
class ShippingService {
  ...
  static async createParcel(orderId, vendorId) {
    ...
    if (config.provider === 'yalidine') {
      ...
      await db.query(
        `UPDATE orders
         SET tracking_code = $1, label_url = $2, shipping_provider = 'yalidine', status = 'قيد التوصيل', shipping_cost_incurred = 1
         WHERE id = $3`,
        [trackingCode, labelUrl, order.id]
      );

      return { tracking_code: trackingCode, label_url: labelUrl };
    }

    if (config.provider === 'zr_express') {
      const items = typeof order.items === 'string' ? JSON.parse(order.items || '[]') : (order.items || []);
      const payload = {
        Colis: [
          {
            Tracking: `ORD-${order.id}`,
            Client: order.customer_name,
            Mobile: order.phone,
            Adresse: `${order.commune} - ${order.address}`,
            Wilaya: order.wilaya_name,
            Commune: order.commune,
            Montant: parseFloat(order.total),
            Stop_Desk: order.delivery_type === 'desk' ? 1 : 0,
            Remarque: items.map((i) => `${i.name} (${i.qty})`).join(', '),
          },
        ],
      };

      const res = await fetch('https://api.zrexpress.com/api/v1/colis', {
        method: 'POST',
        headers: {
      ...
      if (!res.ok || !resData.success) {
        throw new Error(resData.message || 'فشل تسجيل الطرد لدى ZR Express');
      }

      const trackingCode = resData.tracking || `ORD-${order.id}`;
      const labelUrl = `https://api.zrexpress.com/api/v1/print/${trackingCode}`;

      await db.query(
        `UPDATE orders
         SET tracking_code = $1, label_url = $2, shipping_provider = 'zr_express', status = 'قيد التوصيل', shipping_cost_incurred = 1
         WHERE id = $3`,
        [trackingCode, labelUrl, order.id]
      );
    ...
    throw new Error(`مزود الشحن "${config.provider}" غير مدعوم للربط التلقائي`);
  }

  /**
   * التتبع اللحظي لحالة الشحنة (Live Tracking)
   */
  static async trackParcel(orderId, vendorId) {
    const order = await db.get('SELECT * FROM orders WHERE id = $1 AND user_id = $2', [orderId, vendorId]);
    if (!order || !order.tracking_code) throw new Error('لا يوجد رقم تتبع مسجل لهذا الطلب');

    const config = await this.getVendorConfig(vendorId);

    if (order.shipping_provider === 'yalidine') {
      const res = await fetch(`https://api.yalidine.app/v1/histories?tracking=${order.tracking_code}`, {
        headers: {
          'X-API-ID': config.api_key,
          'X-API-TOKEN': config.api_token,
        },
      ...
      if (!res.ok) throw new Error('تعذر جلب سجل التتبع من شركة التوصيل');
      const data = await res.json();
      const lastEntry = data.data?.[0] || {};
      const lastStatus = lastEntry.status || 'In Transit';
      const reason = lastEntry.reason || '';

      let internalStatus = order.status;
      if (['Livré', 'Delivered', 'Payé'].some((s) => lastStatus.includes(s))) {
        internalStatus = 'تم التسليم';
      } else if (['Echoué', 'Failed', 'Tentative échouée'].some((s) => lastStatus.includes(s))) {
        internalStatus = 'تعذر التوصيل';
      } else if (['Retourné', 'Returned', 'Retour', 'Renvoyé'].some((s) => lastStatus.includes(s))) {
        internalStatus = 'مرتجع';
      } else if (['En cours', 'Expédié', 'Centre'].some((s) => lastStatus.includes(s))) {
        internalStatus = 'قيد التوصيل';
      }
